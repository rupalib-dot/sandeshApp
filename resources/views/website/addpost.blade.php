    ...
    <section id="registrationPage" class="registrationPage" role="registration">
        <div class="container">
            <div class="row">
                ...
                <form class="form-sample js-form formContactregi px-0" method="post" action="{{route('addmypost', isset($post->id) ? $post->id : '')}}"
                        ...
                        <div class="col-md-6 col-sm-6 col-12">
                            <div class="form-group mb-4">
                                <select name="swd" class="form-control @error('swd') redborder @enderror" required>
                                    ...
                                    <option {{ Request::old('swd', isset($post) ? $post->swd : '') == 'S/O' ? 'selected' : '' }} value="S/O">S/O</option>
                                    <option {{ Request::old('swd', isset($post) ? $post->swd : '') == 'W/O' ? 'selected' : '' }} value="W/O">W/O</option>
                                    <option {{ Request::old('swd', isset($post) ? $post->swd : '') == 'D/O' ? 'selected' : '' }} value="D/O">D/O</option>
                                    <option {{ Request::old('swd', isset($post) ? $post->swd : '') == 'M/O' ? 'selected' : '' }} value="M/O">M/O</option>
                                    <option {{ Request::old('swd', isset($post) ? $post->swd : '') == 'F/O' ? 'selected' : '' }} value="F/O">F/O</option>
                                    <option {{ Request::old('swd', isset($post) ? $post->swd : '') == 'H/O' ? 'selected' : '' }} value="H/O">H/O</option>
                                </select>
                                @error('swd')
                                    <div class="rederror">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-6 col-sm-6 col-12">
                            <div class="form-group mb-4 showind">
                                <input id="searchTextField" type="text" class="form-control @error('address') redborder @enderror"
                                       ...
                                       placeholder="Location *" name="address" required value="{{isset($post->address) ? $post->address : Request::old('address')}}" >
                    ...
                    <div class="row">
                        <div class="col-md-6 col-sm-6 col-12">
                            <div class="form-group showind mb-4">
                                <input type="text" class="form-control onlydigits @error('number') redborder @enderror"
                                       placeholder="Contact Number *" name="number" required onkeydown="limit(this, 10);" onkeyup="limit(this, 10);"
                                       value="{{isset($post->number) ? $post->number : Request::old('number')}}" pattern="^[0-9]\d{9}$">
                        ...
                        <div class=" col-12">
                            <div class="form-group form-check mb-4 mt-2">
                                <input id="checkgarland" onchange="flowerSelect()" type="checkbox" class="form-check-input @error('flowers') redborder @enderror"
                                       name="flowers" @if(isset($post->flowers) && $post->flowers == 1 || Request::old('flowers')) checked @endif value="1">
                                <label class="form-check-label" for="checkgarland">Do You Want to Customise Image With Garland ?</label>
                                @error('flowers')
                                    <div class="rederror">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6 col-sm-6 col-12" id="flower_type" @if(isset($post->flowers) && $post->flowers == 1 || Request::old('flowers'))  @else style="display:none" @endif>
                            <div class="form-group mb-4">
                                <select name="flower_type" id="flowerTypeSelect" class="form-control @error('flower_type') redborder @enderror"  @if(isset($post->flowers) && $post->flowers == 1 || Request::old('flowers')) required @endif>
                                    <option value="">Select Garland</option>
                                    <option {{ Request::old('flower_type', isset($post) ? $post->flower_type : '') == 'y' ? 'selected' : '' }} value="y">Yellow Garland</option>
                                    <option {{ Request::old('flower_type', isset($post) ? $post->flower_type : '') == 'w' ? 'selected' : '' }} value="w">White Garland</option>
                                    <option {{ Request::old('flower_type', isset($post) ? $post->flower_type : '') == 'p' ? 'selected' : '' }} value="p">Pink Garland</option> 
                                </select>
                                @error('flower_type')
                                    <div class="rederror">{{ $message }}</div>
                                @enderror
                        ...
                        <div class=" col-12">
                            <div class="form-group showind mb-4">
                                <textarea style="resize:none;" class="form-control nofocus @error('description') redborder @enderror"
                                          name="description" minlength="20" maxlength="250" required id="description" @if(isset($post) && $post->template_id != 0) Readonly  @endif
                                          onkeydown="limit(this, 250);" onkeyup="limit(this, 250);"
                            ...
                            <h4>OR</h4>
                            <div class="form-group showind mb-4">
                                <select name="template_id" onchange="getMessage(this.value)" id="messagetemplate" class="form-control @error('template_id') redborder @enderror">
                                    <option value="0">Select Template</option>
                                    @if(count($templates)>0)
                                        @foreach($templates as $template)
                                            <option {{ Request::old('template_id', isset($post) ? $post->template_id : '') == $template->id ? 'selected' : '' }} value="{{$template->id}}">{{$template->message}}</option>
                                        @endforeach
                                    @endif
                                </select> 
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 col-sm-6 col-12">
                            <div class="form-group mb-4 showind">
                                <div class="pt-2">
                                    ...
                                    <div x-data="imageData('{{isset($post->death_certificate) ? asset('storage/uploads/'.$post->death_certificate) : Request::old('death_certificate')}}')" class="file-input flex items-center">
                                        <!-- Preview Image -->
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-5 rounded-md shadow-sm">
                                                ...
                                                <input @change="updatePreview($refs)" x-ref="input"
                                                       ...
                                                       name="death_certificate" id="photo"
                        ...
                        <div class="col-md-6 col-sm-6 col-12">
                            <div class="form-group mb-4 showind">
                                <div class="pt-2">
                                    <!-- If you wish to reference an existing file (i.e. from your database), pass the url into imageData() -->
                                    <div x-data="imageData('{{isset($post->person_pic) ? asset('storage/uploads/'.$post->person_pic) : Request::old('person_pic')}}')" class="file-input flex items-center">
                                        <!-- Preview Image -->
                                        <div class="h-12 w-12 rounded-full overflow-hidden bg-gray-100">
                                            <!-- Placeholder image -->
                                            <div x-show="!previewPhoto" >
                        ...
                        <div class="col-12">
                            <input value="{{isset($post->is_draft) ? $post->is_draft : Request::old('is_draft')}}" id="is_draft" name="is_draft" type="hidden">
                            <button type="submit" class="signUp1 btn createpost btn postadd" style="margin-right: 10px;">{{isset($post->id) ? 'Update' : 'Create'}} Post</button>
